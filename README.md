...
| **图片浏览** | 同目录快速切图、滚轮缩放、拖拽平移，支持适配窗口、实际大小和最近邻 / 双线性采样 |
| **贴图检查** | 查看图片自带的 Mipmap 层级，调节 HDR 曝光，查看尺寸、格式与压缩信息 |
| **动画查看** | GIF、APNG、动态 WebP 播放 / 暂停、逐帧查看与进度控制 |
| **文件操作** | 复制文件路径、打开所在文件夹，确认后将图片移入回收站 |
| **外观设置** | 深浅主题、棋盘格 / 纯色背景、可选磨砂与减少动效；悬浮工具栏无操作 1 秒后自动淡出 |
...
| 文件 | 使用方式 |
| --- | --- |
| **安装版：带 `Setup` 的 `.exe`** | 双击安装，提供开始菜单入口，可选添加“打开方式”和资源管理器缩略图 |
